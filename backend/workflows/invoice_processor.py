...
class InvoiceProcessor:
    ...
    def _build_graph(self) -> StateGraph:
        ...
        workflow.add_node("check_documents", self.check_documents)
        ...
        workflow.add_edge("validate", "check_documents")
    ...
    def check_documents(self, state: InvoiceProcessorState) -> Dict[str, Any]:
        """
        Step 1.5: Verify mandatory documents (Invoice, POD, LR)
        """
        logger.info("Checking mandatory documents")
        
        invoice_data = state.get('invoice_data', {})
        attached = invoice_data.get('attached_docs', [])
        
        required = ['INVOICE', 'POD', 'LR']
        # Normalize to uppercase for comparison
        attached_upper = [doc.upper() for doc in attached]
        missing = [doc for doc in required if doc not in attached_upper]
        
        if missing:
             logger.warning(f"Missing mandatory documents: {missing}")
             
        return {
            "current_step": "check_documents",
            "missing_documents": missing,
            "errors": state.get('errors', []) + ([f"Missing mandatory documents: {', '.join(missing)}"] if missing else [])
        }
    ...
    def assess_risk(self, state: InvoiceProcessorState) -> Dict[str, Any]:
        ...
        Risk Factors:
        - Missing Documents: +1.0 (Critical)
        ...
        # Risk factor 0: Missing Documents (Critical)
        if state.get('missing_documents'):
            logger.info(f"Critical Risk: Missing documents {state.get('missing_documents')}")
            return {
                "risk_score": 1.0,
