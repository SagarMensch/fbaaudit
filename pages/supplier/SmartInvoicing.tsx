...
export const SmartInvoicing: React.FC<SmartInvoicingProps> = ({ supplier }) => {
    ...
    const processFile = async (file: File) => {
        ...
        try {
            ...
            // Call Intelligent Ingestion API (Layer 0-5) with Docling + Mistral
            ...
            console.log('[Ingestion Debug] API Response:', result);

            if (result.extracted_data) {
                const invoice = result.extracted_data;
                console.log('[Ingestion Debug] Invoice data:', invoice);
                ...
                setExtractedData({
                    ...
                    confidence_score: invoice.confidence_score || 0.95,
                    filename: result.filename
                });

                // Store Layout Analysis for debugging/visualization
                const layoutInfo = result.layout_analysis
                    ? JSON.stringify(result.layout_analysis, null, 2)
                    : result.content_markdown;

                setRawText(layoutInfo || null);
                setProcessingTime(result.processing_time_ms || 1200);

                setUploadState('ready');
            } else {
                console.error('[Ingestion Debug] Extraction failed:', result);
                setErrorMessage('Failed to extract invoice data. Please try again.');
                setUploadState('error');
            }
        } catch (error) {
