...
export const PartnerNetwork: React.FC = () => {
   ...
   const getStatusBadge = (status: string) => {
      ...
      switch (s) {
         ...
         case 'suspended':
         case 'inactive':
            return <span className="px-2 py-0.5 text-xs font-bold bg-red-50 text-red-700 border border-red-200 rounded">INACTIVE</span>;
         default:
               ...
               <tbody className="divide-y divide-gray-200">
                  {filteredPartners.map(partner => (
                     <tr key={partner.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4">
                           <div className="text-sm text-gray-700">{partner.connectivity?.type || 'N/A'}</div>
                           <div className="text-xs text-gray-500 mt-0.5">Sync: {partner.connectivity?.lastSync || 'N/A'}</div>
                        </td>
                        <td className="px-6 py-4">
